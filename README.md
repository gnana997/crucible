...
![Status: v0.4.3](https://img.shields.io/badge/status-v0.4.3-orange)
...
- **v0.4.3** (current): **operate & safe-update** — `crucible app update` rolls a new instance out **zero-downtime** (boot → readiness gate → flip the ingress route → drain the old one; a failed update keeps the old instance serving), and you drive a running app **by name** — `app exec`/`logs`/`shell` plus MCP `app_exec`/`app_logs` — resolved to the live instance per call, so it survives a self-heal or redeploy ([docs/apps.md](docs/apps.md)).
