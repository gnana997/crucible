...
func TestToolMirrorByPolicy(t *testing.T) {
	...
	hidden := []string{"run", "create_sandbox", "snapshot", "fork", "delete_sandbox", "delete_snapshot", "delete_app", "delete_image", "capture", "volume_create", "delete_volume"}
	for _, n := range shown {
		if !got[n] {
...
func TestToolMirrorFullPolicyAdvertisesAll(t *testing.T) {
	...
	if len(res.Tools) != 30 {
		t.Errorf("all-ops policy advertised %d tools, want 30", len(res.Tools))
	}
}
