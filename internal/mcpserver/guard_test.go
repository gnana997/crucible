...
func TestToolMirrorByPolicy(t *testing.T) {
	...
	shown := []string{"exec", "app_exec", "app_sleep", "app_wake", "app_stop", "app_start", "write_files", "read_file", "stop_sandbox", "logs", "app_logs", "list_sandboxes", "inspect_sandbox", "list_snapshots", "list_profiles", "list_images", "list_volumes", "create_app", "update_app", "list_apps", "get_app", "app_domain_add", "app_domain_rm", "app_domain_ls"}
...
func TestToolMirrorFullPolicyAdvertisesAll(t *testing.T) {
	...
	if len(res.Tools) != 37 {
		t.Errorf("all-ops policy advertised %d tools, want 37", len(res.Tools))
	}
}
