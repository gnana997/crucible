...
func TestToolMirrorByPolicy(t *testing.T) {
	...
	shown := []string{"exec", "app_exec", "app_sleep", "app_wake", "write_files", "read_file", "stop_sandbox", "logs", "app_logs", "list_sandboxes", "inspect_sandbox", "list_snapshots", "list_profiles", "list_images", "create_app", "update_app", "list_apps", "get_app"}
	hidden := []string{"run", "create_sandbox", "snapshot", "fork", "delete_sandbox", "delete_snapshot", "delete_app", "delete_image", "capture"}
	for _, n := range shown {
		if !got[n] {
...
func TestToolMirrorFullPolicyAdvertisesAll(t *testing.T) {
	...
	if len(res.Tools) != 27 {
		t.Errorf("all-ops policy advertised %d tools, want 27", len(res.Tools))
	}
}
