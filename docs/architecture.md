...
`internal/app` holds each app's **desired state** in a bbolt store and runs a level-triggered reconcile loop that converges the actual instance toward it (see [apps.md](apps.md)). On daemon start — *after* the sandbox reap above has torn down the previous run's VMs — the loop boots a fresh instance for every desired-running app from its stored spec, which is how an app survives a restart (re-created, not re-attached). While running it restarts a dead or unhealthy instance with exponential backoff + a crash-loop guard, and probes http/tcp health checks. An `app update` is diff-aware: a change touching only host-side state (sleep policy, `can_call`, health, restart policy, metrics scrape) is applied **in place** — the record's `spec_revision` moves, its generation doesn't, and the daemon picks the new values up from the record (the idle evaluator, call authorization, and prober read it live; the waking forwarders and internal VIPs are re-asserted from spec each reconcile pass) — while an instance-defining change bumps the stored spec's generation; for a proxy-fronted app the reconciler rolls that out **zero-downtime** — it boots the new instance without flipping, waits for a readiness gate (health check, or a TCP connect to the app's port), then flips which instance is current and drains the old one; the ingress resolver follows the flip with no proxy-side coordination, and a new instance that never becomes ready is abandoned with the old one left serving. The app's instance is an ordinary sandbox created through the same path a `POST /sandboxes` uses (the daemon supplies `internal/app` an *instantiator* over `sandbox.Manager`), so exec/logs/snapshot work on it unchanged — and `POST/GET /apps/{name}/exec` + `GET /apps/{name}/logs` resolve the app to its current instance server-side so those operate by name.
