...
- [x] **Ingress proxy** — reach an app by name instead of a published port. `--proxy-listen` (Host-header routing, L7), `--proxy-tls-listen` (SNI passthrough, L4 — the guest terminates its own TLS), `--proxy-domain <domain>` (`web.<domain>` → app `web`). Off in the daemon by default; the installer enables it on the high ports `:8080`/`:8443` (leaving `:80`/`:443` free for `-p`/`-P`), overridable to `:80`/`:443` for a production ingress. Resolution is live, so the route follows the app across self-heal and redeploy and never points at a stale IP ([proxy.md](proxy.md)).
