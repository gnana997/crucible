...
Each persistent volume can be its own encrypted container with its own key, so a
tenant's data can be crypto-shredded by destroying that key.
...
The app activity timeline — and the exact sleep/wake timing that makes billing precise.
