...
- **Detached-only.** The volume must be detached (stop the app first) — a `409` otherwise, because a snapshot-slept volume's guest has its device size pinned by the snapshot until it next boots. The shared resize runs `e2fsck` first, so a volume detached from a hard-killed guest resizes cleanly. `POST /volumes/{name}/grow`, SDK `GrowVolume`.
